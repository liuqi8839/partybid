...
function get_prices(){
    ...
    for(var i=0;i<price.length;i++){
        if(price[i].activity==get_selected_activity()) {
...
function creat_new_price(){
    ...
    unshift_value({"activity":get_selected_activity(),"count":new_count(),"status":1,"selected":1},prices,'Price');
}
...
function determine_StartButton(){
    if(get_ongoing_price().activity=='null' ||get_ongoing_price().activity=='never'){
        ...
        if(ongoing_price.activity==get_selected_activity()){
...
function price_select(count) {
    ...
    for(var i=0;i<prices.length;i++){
        prices[i].selected=2;
        if(prices[i].count==count && get_selected_activity()==prices[i].activity){
            prices[i].selected=1;
        }
    }
...
function judge_sign_up(){
    ...
    for(var i=0;i<sign_up_information.length;i++) {
        if (sign_up_information[i].activity == get_selected_activity()) {
            flag=2;
            break;
        }
    }
